...
- Use `$evidence-graph` when changing a package's evidence stage, writing or reviewing evidence tags, handling fingerprints, deciding coverage or exclusions, editing evidence configuration, or diagnosing `ttsc`.
...
- Use `$edit-agent-skills` before creating, moving, splitting, or changing any repository skill.
...
- Repository skills live at `.agents/skills/<name>/SKILL.md`. A substantial skill may keep directly linked Markdown phase instructions beside its `SKILL.md`; do not create deeper skill subdirectories or unreferenced auxiliary files.
...
- Principle references are compiler checklists: each file answers each anchored H2 item individually — `common.md` plus the layer file for narrative files, `settings.md` for settings files. Whole-file citations are refused, `common.md` items can never be excluded, and a layer item is excluded only with a concrete scope reason.
...
Run `pnpm --filter <package-name> ttsc` during scoped work. Run `pnpm ttsc` from the workspace root and `git diff --check` before completing document, skill, manifest, package, or evidence-config changes. Report unresolved diagnostics instead of weakening the graph.
