...
A commander reads a map through scale, symbols, survey date, and purpose. A village may have moved, a road may be seasonal, and a contour may hide a ravine. Scenarios must distinguish map knowledge from sight and report, and must identify whose map is available at the moment of action.
...
The emperor can issue an order that reaches a subordinate after the tactical situation has changed. A family letter may arrive after illness or death. Every unit must record dispatch, route, receipt, and response when those facts are known, and must preserve uncertainty when a source gives only one end of the chain.
...
A source's “morning,” “nine,” or date may describe observation, order, arrival, or publication. The novel must preserve the source's precision and never synchronize incompatible clocks without explanation. Later documents must state whether a time is local, staff, astronomical, estimated, or converted by an editor.
...
A signal can be heard without being understood, delayed by terrain, or mistaken by a neighboring unit. A courier can know the order but not the situation awaiting him. Settings must identify sender, medium, route, receiver, code, and confirmation rather than treating communication as instantaneous command.
...
The date on a printed collection does not necessarily equal the date of composition or receipt. Later editors can regularize spelling, omit enclosures, or supply a conversion. The evidence graph must retain document identity, edition, audience, and editorial intervention before assigning a precise time or quotation.
...
One headquarters may know an enemy has moved while a corps still acts on yesterday's order. Local guides may know a ford that a printed map omits; a commander may distrust a true report because its source has an incentive to mislead. Later scenes must show what was available, what was believed, and what remained unknown at the decision point.
...
No file may invent a fingerprint or claim review merely because its build passes. When the settings stage advances, every H2 will be checked against source attribution, contradiction, precision, and downstream lineage. The graph state is a compiler condition, not a substitute for reading the document.
...
Later layers must cite the map, dispatch, signal, clock, and observer they use. They may stage a failed message or delayed report only when the settings canon supplies its physical route and information limit; they may not turn a settings constraint into an ungrounded dramatic shortcut.

Sources: repository `config/docs/principles/common.md`; repository `.agents/skills/novel/settings.md`.
