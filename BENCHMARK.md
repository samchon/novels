...
- **Layer-activation gate**: no layer moves off `disabled` while `@ttsc/evidence` 0.28.0 is unpublished. The overrides bind the graph to a locally packed build, so a layer activated before the release is compiled against something no one else can install; the registry is checked at every activation and the transition waits, rechecking every fifteen minutes, until 0.28.0 resolves. `settings` on `napoleon-imperator` had been advanced to `review` in an earlier session, before this rule existed. It was returned to `disabled` rather than left running, because the gate is about what the graph is compiled against, not about when the rule was written. The evidence and review tags stay in the files; only enforcement is off until the release lands.
